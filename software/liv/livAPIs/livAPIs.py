...
@app.route('/getAllRawSensorData')
def getAllRawSensorDataFromMostRecentReading():
    print "start getAllRawSensorData"
    records = getData(1)
    c = records[0]
    t = c["Temperature"]
    h = c["Humidity"]
    ...
    ts = c["Timestamp"]
    co2 = c["CO2level"]
    return jsonify({ 'Temperature': str(t), 'Humidity': str(h), 'AirPressure': str(ap), 'Timestamp': ts, 'CO2level': str(co2)})
